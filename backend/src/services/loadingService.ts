...
export class LoadingService{
 constructor(private readonly repo:LoadingRepository){}
 ...
 create(loadId:string,input:Settings){this.validateDestination(input);const existing=this.repo.findByLoad(loadId);if(existing)return existing;return this.repo.transaction(()=>this.repo.create(loadId,input));}
 update(id:string,input:Settings){this.get(id);this.validateDestination(input);return this.repo.transaction(()=>this.repo.update(id,input));}
