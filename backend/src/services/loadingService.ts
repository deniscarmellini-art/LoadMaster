...
export class LoadingService{
 ...
 ship(id:string,input:{carrierId?:string}){const session=this.get(id);if(!["IN_CARICO","ATTESA_SPEDIZIONE"].includes(session.stato))throw new ApiError(409,"INVALID_STATUS","Carico non spedibile");if(session.destinationType==="TRASPORTATORE"&&!input.carrierId)throw new ApiError(400,"VALIDATION_ERROR","Trasportatore obbligatorio");if(session.stato==="IN_CARICO"&&!this.repo.isComplete(id,session.loadId))throw new ApiError(409,"LOADING_INCOMPLETE","Carico incompleto");return this.repo.transaction(()=>this.repo.ship(id,input.carrierId));}
