...
export interface LoadingSessionRecord{transportMode:"BILICO_ESSEPI"|"RITIRA_CLIENTE"|"TERZI_PER_ESSEPI"|null;transportDetailId:string|null;transportDetailLabel:string|null;id:string;loadId:string;commessa:string;cliente:string;camion:string;stato:LoadingStatus;operatorId:string;destinationType:DestinationType;trailerId:string|null;carrierId:string|null;startedAt:string;completedAt:string|null;reopenedAt:string|null;shippedAt:string|null;createdAt:string;updatedAt:string;units:LoadingUnitRecord[];events:Array<{type:string;operatorId:string|null;timestamp:string;note:string|null}>;}
