...
test("le prenotazioni manuali persistono e vengono convertite nel carico reale",async()=>{
  ...
  try{const first=await buildApp(persistentConfig);const trailers=(await first.inject({method:"GET",url:"/api/trailers"})).json<Array<{id:string}>>(),operator=(await first.inject({method:"GET",url:"/api/operators"})).json<Array<{id:string}>>()[0]!;const trailer=trailers[0]!,freeTrailer=trailers[1]!;
    ...
    const edited=await first.inject({method:"PUT",url:`/api/trailers/${trailer.id}/reservation`,payload:{commessa:"COMM-TEST",cliente:"Cliente Test aggiornato",carico:"C1",plannedDepartureDate:"2026-09-16"}});const editedReservation=edited.json<{cliente:string;plannedDepartureDate:string}>();assert.equal(editedReservation.cliente,"Cliente Test aggiornato");assert.equal(editedReservation.plannedDepartureDate,"2026-09-16");
