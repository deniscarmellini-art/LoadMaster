...
export class CarrierRepository{
 ...
 setActive(id:string,active:boolean):Carrier|null{if(!this.find(id))return null;this.database.prepare("UPDATE Carriers SET active=?,updatedAt=? WHERE id=?").run(active?1:0,new Date().toISOString(),id);return this.find(id);}
