...
export default function PrintLabels({ commesse, listeOperative, onBack }:Props) {
  ...
  const formatNumber=(value:number,maximumFractionDigits=4)=>value.toLocaleString("it-IT",{maximumFractionDigits});
  ...
  return <Box>
    ...
    {commessa&&<Paper className="no-print" sx={{p:{xs:1.5,sm:2.5},mb:2}}><Typography variant="h6" sx={{mb:2}}>2. Elementi da stampare</Typography><Stack direction={{xs:"column",sm:"row"}} sx={{alignItems:{xs:"stretch",sm:"center"},gap:1,mb:1.5,flexWrap:"wrap"}}><Button size={mobile?"large":"medium"} variant="outlined" onClick={()=>setSelected(new Set(panels.map((_,i)=>i)))}>Seleziona tutti</Button><Button size={mobile?"large":"medium"} variant="outlined" onClick={()=>setSelected(new Set())}>Deseleziona tutti</Button>{trucks.length>0&&<Stack direction="row" sx={{alignItems:"center",gap:1,ml:{xs:0,sm:1},flexWrap:"wrap"}}><Typography variant="body2" color="text.secondary">Seleziona carico:</Typography>{trucks.map(truck=><Button key={truck} variant="outlined" size={mobile?"medium":"small"} onClick={()=>selectTruck(truck)}>{truck}</Button>)}</Stack>}</Stack><Typography color="text.secondary" sx={{mb:1.5,fontWeight:700}}>{selected.size} {selected.size===1?"elemento selezionato":"elementi selezionati"}</Typography>
      {!mobile?<TableContainer sx={{maxHeight:460}}><Table stickyHeader size="small"><TableHead><TableRow>{["","Elemento","Master panel","Tipologia","Camion","Spessore","Lunghezza","Altezza","Peso (kg)"].map(h=><TableCell key={h}>{h}</TableCell>)}</TableRow></TableHead><TableBody>{panels.map((panel,index)=><TableRow hover key={index}><TableCell><Checkbox checked={selected.has(index)} onChange={()=>toggle(index)}/></TableCell><TableCell><b>{panel.numeroPannello}</b></TableCell><TableCell>{panel.numeroMasterPanel}</TableCell><TableCell>{panel.tipoPannello||"—"}</TableCell><TableCell>{panel.numeroCamion}</TableCell><TableCell>{formatNumber(panel.spessore)}</TableCell><TableCell>{formatNumber(panel.lunghezza)}</TableCell><TableCell>{formatNumber(panel.altezza)}</TableCell><TableCell sx={{whiteSpace:"nowrap"}}>{formatNumber(panel.peso)} kg</TableCell></TableRow>)}</TableBody></Table></TableContainer>:<Stack sx={{gap:1.25}}>{panels.map((panel,index)=><Paper key={`${panel.numeroPannello}-${index}`} variant="outlined" sx={{p:1.5,borderColor:selected.has(index)?"primary.main":"divider"}}><Stack direction="row" sx={{alignItems:"center",gap:1,mb:1.25}}><Checkbox checked={selected.has(index)} onChange={()=>toggle(index)} slotProps={{input:{"aria-label":`Seleziona elemento ${panel.numeroPannello}`}}} sx={{p:.5,"& .MuiSvgIcon-root":{fontSize:32}}}/><Box sx={{minWidth:0}}><Typography sx={{fontWeight:900,fontSize:"1.05rem"}}>Elemento {panel.numeroPannello}</Typography><Typography variant="body2" color="text.secondary">Master Panel {panel.numeroMasterPanel} · {panel.tipoPannello||"Tipologia —"} · Camion {panel.numeroCamion}</Typography></Box></Stack><Box sx={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:1}}><Box><Typography variant="caption" color="text.secondary">Spessore</Typography><Typography>{formatNumber(panel.spessore)}</Typography></Box><Box><Typography variant="caption" color="text.secondary">Lunghezza</Typography><Typography>{formatNumber(panel.lunghezza)}</Typography></Box><Box><Typography variant="caption" color="text.secondary">Altezza</Typography><Typography>{formatNumber(panel.altezza)}</Typography></Box><Box><Typography variant="caption" color="text.secondary">Peso</Typography><Typography>{formatNumber(panel.peso)} kg</Typography></Box></Box></Paper>)}</Stack>}
    </Paper>}
