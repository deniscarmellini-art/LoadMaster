...
interface Props { managedFocus?:boolean; onRestoreFocus?:()=>void; disabled?:boolean; inputRef?:RefObject<HTMLInputElement|null>; mobileEmphasis?:boolean; value:string; onValueChange:(value:string)=>void; onScan:(value:string)=>void; }
export default function ScannerInput({ managedFocus=false, onRestoreFocus, disabled=false, inputRef:externalInputRef, mobileEmphasis=false, value, onValueChange, onScan }:Props) {
  ...
  return <>
    <Stack direction={{xs:"column",sm:"row"}} sx={{gap:1.5,alignItems:"stretch"}}>
      {!smartphone && (
        <TextField autoFocus={!managedFocus} disabled={disabled} fullWidth inputRef={inputRef} label="Scanner QR elemento" placeholder="Scansiona il QR e premi Invio" value={value} onChange={e=>onValueChange(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();submit();}}} slotProps={{input:{startAdornment:<InputAdornment position="start"><QrCodeScannerIcon sx={{fontSize:34}}/></InputAdornment>}}} sx={{order:{xs:mobileEmphasis?2:1,sm:1},"& .MuiOutlinedInput-root":{fontSize:"1.15rem",minHeight:{xs:mobileEmphasis?56:72,sm:72}},"& input":{fontFamily:"monospace"}}}/>
      )}
