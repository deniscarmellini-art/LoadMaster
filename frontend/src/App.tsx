      ...
      <MainLayout>
        {loadsLoading ? <Box sx={{display:"flex",justifyContent:"center",py:8}}><CircularProgress/></Box> : loadsError ? <Alert severity="error">{loadsError}</Alert> : page === "dashboard" ? (
          <Dashboard
            ...
            packages={packages}
            onReopenLoad={(row,operator:Operatore,reason)=>{const existing=truckLoads.find(load=>load.commessa===row.commessa&&load.camion===row.camion&&load.stato==="ATTESA_SPEDIZIONE");if(existing)void reopenLoadingApi(existing.loadId,reason).then(async()=>{await refreshScanningData();const value={...existing,stato:"IN_CARICO" as const,eventi:[...existing.eventi,{tipo:"RIAPERTURA" as const,dataOra:new Date().toISOString(),operatoreId:operator.id,operatore:operatorLabel(operator),nota:reason||undefined}]};setResumeLoad(value);setPage("loading");});}}
            onContinueLoad={resumeLoadingSession}
            onStartLoad={(row)=>{const load:CaricoCamion={loadId:"",backendLoadId:row.id,commessa:row.commessa,cliente:row.cliente,camion:row.camion,stato:"DA_CARICARE",operatoreId:"",operatore:"",avviatoIl:"",scansioni:[],eventi:[]};setResumeLoad(load);setLoadingInstance(value=>value+1);setPage("loading");}}
